...
import { addAuthorization, editAuthorization, setAuthorizations, removeAuthorization } from "../actions/creators"
...
describe('the authorizations reducer', () => {
  ...
  it('set auth', () => {
    let newState = authsReducer(mockState(), setAuthorizations(RemoteDataState.NotStarted))
    expect(newState.status).toBe(RemoteDataState.NotStarted)

    newState = authsReducer(newState, setAuthorizations(RemoteDataState.Loading))
