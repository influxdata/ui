...
export const updateTaskName =
  (name: string, taskID: string) => async (dispatch: Dispatch<Action>) => {
    try {
...
export const updateTask =
  ({script, interval, preamble, task}: UpdateTaskParams) =>
...
const cloneTaskLabels =
  (sourceTask: Task, destinationTask: Task) =>
  async (dispatch: Dispatch<Action>) => {
    try {
...
export const setAllTaskOptionsByID =
  (taskID: string) =>
  async (dispatch: Dispatch<Action>): Promise<void> => {
    try {
      const resp = await api.getTask({taskID})
...
export const goToTasks =
  () => (dispatch: Dispatch<Action | RouterAction>, getState: GetState) => {
