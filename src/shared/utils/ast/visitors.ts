...
import {
  isRangeNode,
  constructWindowVarAssignmentFromRange,
} from 'src/shared/utils/ast'

/*
  Legacy function.

  Find all nodes in a tree matching the `predicate` function.
    * Each node in the tree is an object.
    * Each object which may contain objects or arrays of objects as children, under any key.
*/
 ...
 * @param halt -- halt condition
 * @param whenFound -- callback triggered when halt condition is met
 * @param {Accumulator} acc -- accumulator
 * @returns {Accumulator} -- return Accumulator, including scope
 */
export const findNodeScope = (
  ...
  switch (node.type) {
    ...
    default:
      return acc
    // Uncomment to below line, to have typescript check for exhuastive switch.
