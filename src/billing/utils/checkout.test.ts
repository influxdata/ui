...
const testCases: ({ snakeCase: stringDict, camelCase: stringDict })[] = [
  ...
  { camelCase: { testing: "some string" }, snakeCase: { testing: "some string" } },
  {
    ...
    snakeCase: { numbers_01_test: "some string", num_ber_1_test_2_blah: "other string" }
  },
]


describe("checkout", () => {
  testCases.forEach(({ camelCase, snakeCase }, i) => it(`converts object both ways ${i}`, () => {
  ...
  it(`converts abbreviations`, () => {
    expect(convertKeysToSnakecase({ UPPERCASEABBREVIATIONS: "some string" }))
      .toMatchObject({ uppercaseabbreviations: "some string" })
    expect(convertKeysToCamelCase({ UPPERCASEABBREVIATIONS: "some string" }))
      .toMatchObject({ uppercaseabbreviations: "some string" })
  })
})
