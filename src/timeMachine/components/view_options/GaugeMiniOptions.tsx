...
// Components
import { Button, ButtonShape, FormElement, Grid, Input, InputType, MultiSelectDropdown, SelectDropdown } from '@influxdata/clockface'
...
import { DEFAULT_GAUGE_COLORS, THRESHOLD_COLORS } from "src/shared/constants/thresholds";
...
import { GAUGE_MINI_THEME_BULLET_DARK, GAUGE_MINI_THEME_PROGRESS_DARK } from "src/shared/constants/gaugeMiniSpecs";
import { getGroupableColumns } from "src/timeMachine/selectors";
import { ComponentStatus } from "src/clockface";

type ThemeString = "GAUGE_MINI_THEME_BULLET_DARK" | "GAUGE_MINI_THEME_PROGRESS_DARK";
...
export const setGaugeMiniProp = (props: Partial<GaugeMiniLayerConfig> & {
  defaultTheme?: ThemeString,
...
class GaugeMiniOptions extends PureComponent<Props> {
  public render() {
    ...
    const renderFields = (fields: Field[]) => fields.map(x => {
      switch (x.type) {
        case "element":
          return x.jsx;
        ...
        case "number":
          return <FormElement label={x.label}>
            <Input
          ...
          return <>
            <FormElement label={x.label}>
              <ColorDropdown
                ...
                onChoose={({ hex }) => { onUpdateProp({ [x.name]: hex }) }}
              />
            </FormElement>
          </>
      }
    ...
    const columns =
      Object.keys(theme.barsDefinitions.groupByColumns)
        .filter(x => theme.barsDefinitions.groupByColumns[x])

    const onSelectMulti = (option: string) => {
      const res = (columns.some(x=>option===x))
      onUpdateProp({barsDefinitions:{
    ...
    const grupby = <FormElement label="Group By">
      <MultiSelectDropdown
    ...
    const defaultThemesSelector = <Row>
      <Column widthXS={8}>
        <SelectDropdown
          options={["GAUGE_MINI_THEME_BULLET_DARK", "GAUGE_MINI_THEME_PROGRESS_DARK"] as ThemeString[]}
          selectedOption={this.props.defaultTheme}
          onSelect={x => { onUpdateProp({ defaultTheme: x as ThemeString }) }}
        />
      </Column>
      <Column widthXS={4}>
        <Button
    ...
    return (
      <>
        ...
        {renderFields([
          ...
          { type: "select", name: "textMode", options: ["follow", "left"] as GaugeMiniLayerConfig["textMode"][], label: "textMode" },
          {
            type: "element", jsx:
              <FormElement label="Bar definitions">
                {
                  grupby
                }
              </FormElement>
          ...
          { type: "number", name: "labelMainFontSize", label: "labelMainFontSize" },
          { type: "color", name: "labelMainFontColor", label: "labelMainFontColor" },

          { type: "heading", label: "Bar labels" },
          { type: "number", name: "labelBarsFontSize", label: "labelBarsFontSize" },
          { type: "color", name: "labelBarsFontColor", label: "labelBarsFontColor" },
          ...
          { type: "color", name: "valueFontColorInside", label: "valueFontColorInside" },
          { type: "color", name: "valueFontColorOutside", label: "valueFontColorOutside" },
          {
            type: "element", jsx:
