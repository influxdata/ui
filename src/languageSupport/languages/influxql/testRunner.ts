...
export const testTokenization = (_language: string, tests: TestItem[][]): void => {
...
function runTest(languageId: string, test: TestItem[]): void {
  ...
  console.log({languageId, test, test_tokens: test[0].tokens, text, actualTokens: actual[0].tokens})
}