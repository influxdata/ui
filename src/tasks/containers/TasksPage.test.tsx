...
jest.mock('src/client', () => ({
  getTasks: jest.fn(() => {
   return {
      data: {
        tasks: localTasks.map(t => {t.id, t.orgID, t.name, t.flux, t.labels})
      },
      headers: {
      }
    ,
...
describe('tasks page rendering',  () => {
  ...
  describe('view tasks', () => {
    it('hides inactive tasks', async () => {
      ...
      let alerts = (await screen.getByTestId('page-contents'))
      ...
      alerts = (await screen.getByTestId('page-contents'))
        .getElementsByClassName('hidden-tasks-alert')
  ...
  describe('create tasks', () => {
    it('triggers create a new task', async () => {
      ...
      await waitFor(() => {
        expect(screen.queryByTestId('add-resource-dropdown--import')).toBeVisible()
      })
