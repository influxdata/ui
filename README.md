...
1. to run all tests [in series] locally, and generate a report:
    1. `yarn test:e2e:remocal:report` or `yarn test:e2e:remocal:iox:report`
    1. this will run in headless mode, so no browser will be shown. Just a stdout waiting as each test runs.
        * **WARNING**: This takes a long time, unlike CI which runs in parallel.
    1. after all tests complete, it will automatically open a test result viewer.
    1. any screenshots or vids will be saved in `cypress/videos` and `cypress/screenshots`
